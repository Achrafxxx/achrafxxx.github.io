...
jQuery(function ($) {
  ...
  SHIPPING_SETTINGS.sendMessageAJAX = function () {
    $('#c_send').on('click', function () {
      if ($(this).hasClass('disabled')) {
        ...
        xmlhttp.send(
          'email=' + encodeURIComponent(email) + '&name=' + encodeURIComponent(name) + '&msg=' + encodeURIComponent(msg)
        );
        return false;
      }
    });
  ...
  function enable_form() {
    if ($('#c_name').hasClass('green') && $('#c_email').hasClass('green') && $('#c_message').hasClass('green')) {
